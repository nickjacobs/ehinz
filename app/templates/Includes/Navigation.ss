	...
	<div class="container">
		<div class=" collapse navbar-collapse" id="mainNavigation">
			<ul class="navbar-nav w-100 ">
				...
				<% loop Menu(1) %>
					<% if $Children %>	  
						<li class="nav-item dropdown url-{$URLSegment} $FirstLast">
							<a class="nav-link dropdown-togglex" title="{$MenuTitle}" href="$Link" id="navbarDropdown{$ID}" role="button"  aria-haspopup="true" aria-expanded="false">
								$MenuTitle.XML <span class="tereo">$TeReoTitle</span><% if $LinkingMode=='current'%><span class="sr-only">(current)</span><% end_if %>
							</a>
					...
					<% else %>
						<li class="nav-item url-{$URLSegment}">
							<a class="nav-link" title="{$MenuTitle}" href="$Link">$MenuTitle.XML <span class="tereo">$TeReoTitle</span><% if $LinkingMode=='current'%><span class="sr-only">(current)</span><% end_if %></a>
						</li>
				<% end_if %>
				<% end_loop %>
			</ul>
